
module.exports		= (function(){
	
	var Quizz		= function(io, lobby_ref,data){
		this.$io				= io;
		this.$items 			= new Array();
		this.$players			= new Array();
		this.$spectators		= new Array();
		this.$game_reference 	= "QUIZZ_"+Math.round(new Date().getTime() / 1000);
		
		this.$player_room		= this.$game_reference + '_players';
		this.$spectator_room 	= this.$game_reference + '_spectators';
		this.$lobby_reference	= lobby_ref;
		this.init();
		console.log("-------- Quizz game created ----------");
	};
	//Variables
		var $io;
		var $player_room;
		var $spectator_room;
		var $items;
		var $players;
		var $spectators;
		var $game_reference;
		var $game_name;
		var $current_item;
		var $lobby_reference;
		var $server;

	
	
	
	Quizz.prototype	= function(){
		var numseq = 0;

		var load_item_list	= function(){
			this.$items = new Array(
				 {id	: 1, item	: 'あ', answer	: 'a'}
				,{id	: 2, item	: 'い', answer	: 'i'}
				,{id	: 3, item	: 'う', answer	: 'u'}
				,{id	: 4, item	: 'え', answer	: 'e'}
				,{id	: 5, item	: 'お', answer	: 'o'}
				,{id	: 6, item	: 'か', answer 	: 'ka'}
				,{id	: 7, item	: 'き', answer 	: 'ki'}
				,{id	: 8, item	: 'く', answer 	: 'ku'}
				,{id	: 9, item	: 'け', answer 	: 'ke'}
				,{id	: 10, item	: 'こ', answer 	: 'ko'}
			);
		};
		
		var init		= function(){
			numseq++;
			this.$game_name	= "QUIZZ "+numseq;
			this.load_item_list();
		};
		
		var emit_item	= function(self){
			var i = (Math.floor(Math.random() * 10));
			console.log("INFO : emit_item : "+i);
			console.log(self.$items);
			
			self.$current_item = self.$items[i];
			console.log("INFO : room : "+self.$player_room);
			//emit id & item to every player in the room
			self.$io.to(self.$player_room).emit('quizz_item',{item : self.$items[i].item, 
																	id : self.$items[i].id});
			//emit item to every spectator
			self.$io.to(self.$spectator_room).emit('spectator_item',self.$items[i].item);
		};
		
		
		var validate_answer	= function(socket,id,answer){
			var correct = false;
			
			//if the user is trying to answer to the right item
		 	if(this.$current_item.id == id){
				correct = (this.$current_item.answer === answer);
			}
			else {
				console.log("* WARN : user answer has incorrect item id.");
			}
			
			socket.emit('validation',correct);
		};
		
		
		//Only Add the player to the player list. Nothing else.
		var only_add_player		= function(player,server,self){
			console.log("------ Testing self 3 --------");
			console.log(self);
		//if the player doesn't already exist, add him		
			if(self.$players[player.reference] == undefined){
				self.$players[player.reference] = player;
				console.log("Player added : "+player.get_name());
				self.$io.to(self.$lobby_reference).emit('lobby_info', server.get_lobbies_data());
			}
			else{
				console.log("* WARN : Player "+player.get_reference()+" already exists.");
				console.log("Player not added : "+player.get_name());
				console.log("Reference used : "+player.reference);
				//output or log a warning, because we tried to add a user
				//that already existed. 
			}
		};
		
		var add_player			= function(data){
			var player 	= data.user;
			var self	= data.self;
			var server	= data.server;
			var lobby 	= player.get_lobby();
			
			console.log("------ Testing self 2 --------");
			console.log(self);
			
			//add the player to the player list
			self.only_add_player(player,server,self);
			//remove the player from the lobby
			lobby.remove_player(player.get_reference(),server);
		};
		
		//Add a spectator to the spectator list
		var add_spectator		= function(spectator){
			//if the spectator doesn't already exist, add him
			if(this.$spectator[spectator.reference] == undefined){
				this.$spectator[spectator.reference] = spectator;
			}
			else{
				//output or log a warning, because we tried to add a spectator
				//that already existed.
				console.log('* WARN : Spectator '+spectator.get_name()+' already exists.');
			}
		};
		
		//remove a player from the player list
		var remove_player		= function(player,self){
			console.log(player);
			self.$io.to(self.$spectator_room).emit('message','* Player removed : '+player.get_name());
			console.log(self.$players);
			delete self.$players[player.reference];
			
			//Now that the player is removed from the game, add him back to the lobby
			player.get_lobby().add_player(player);
		};
		
		//remove a spectator from the spectator list
		var remove_spectator	= function(spectator,self){
			self.$io.to(self.$player_room).emit('message','* Player removed : '+spectator.get_name());
			delete self.$spectators[spectator.reference];
			
			//Now that the player is removed from the game, add him back to the lobby
			spectator.get_lobby().add_player(spectator);
		};
		
		var leave_game			= function(player,self){
		//remove the player from the lobby
			remove_player(player,self);
			remove_spectator(player,self);
			
		//emit to the lobby, so that they can reload their information
			self.$io.to(self.$lobby_reference).emit('lobby_info', self.$server.get_lobbies_data());
			
		};
		
		var get_player_room_reference 		= function(){
			return this.$player_room;
		};
		
		var get_spectator_room_reference 	= function(){
			return this.$spectator_room;
		};
		
		var get_reference 					= function(){
			return this.$game_reference;
		};
		
		var get_name 					= function(){
			return this.$game_name;
		};
		
		var get_players			= function(){
			return this.$players;
		};
		
		var get_spectators		= function(){
			return this.$spectators;
		};
		
		var on				= function(io,socket,data){
			console.log("---------- Dans game.on() ------------");
			
			this.$server = data.server;
			
			
			switch(data.action){
				case 'game_start'	: 	emit_item(this);
					break;
					
				case 'answer' 		: 	validate_answer(socket,data.item_id,data.answer,this);
					break;
				
				case 'join_game'	: 	data.self = this;
										add_player(data);
					break;
					
				case 'leave_game'	: 	leave_game(data.user,this);
										data.server.get_lobbies_data();
					break;
					
				default : console.log("* ERR: An unknown action has been received : "+data.action);
			}
		};
		
		return {
			load_item_list					: load_item_list
			//,emit_items						: emit_items
			,validate_answer				: validate_answer
			,only_add_player				: only_add_player
			,add_player						: add_player
			,add_spectator					: add_spectator
			,remove_player					: remove_player
			,remove_spectator				: remove_spectator
			,get_player_room_reference		: get_player_room_reference
			,get_spectator_room_reference	: get_spectator_room_reference
			,get_reference					: get_reference
			,get_players					: get_players
			,get_spectators					: get_spectators
			,get_name						: get_name
			,init							: init
			,on								: on
		}; 
	}();
	
	return{
		Quizz	: Quizz
	};
})();
