	...
	************************************************/
	...
	//when a user enters a lobby
		...
		var player_joins_quizz		= function(quizz,lobby_ref,user){
			...
			console.log();
			console.log();
			console.log("------ Testing self 1 --------");
			console.log(quizz);
			quizz.add_player({user:user,server:self,self:quizz});
			
			
			
			//remove the player from the lobby
			self.lobbies[lobby_ref].remove_player(user.get_reference(),self);
		};
		...
		var get_lobby	= function(reference){
			...
			if(lobby != undefined){
				console.log("Lobby found : "+lobby.get_reference());
			}
			else{
		...
		var events_handling			= function(){
			...
			io.sockets.on('connection',function(socket){
				...
				socket.on('QUIZZ',function(data){
					...
					//put the self item in the data object. Momentary measure
					data.server = self;
					data.lobby 	= user.get_lobby();
					//get the corresponding game
					var game		= data.lobby.get_game(game_reference);
					
					if(game != undefined){
						game.on(io,socket,data);
					}
					else{
						console.log("[Lobby "+data.lobby.get_name()+"]Could not find the game : "+game_reference);
					}
				});
			});
		};
