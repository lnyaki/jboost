...
var Quizz = function(){
	...
	************************************************/
	...
	//when a user enters a lobby
		var player_enters_lobby 	= function(socket,lobby_ref,user_ref){
			console.log(user_ref+' enters in lobby : '+lobby_ref);
			socket.join(lobby_ref);
			io.to(lobby_ref).emit('message', "User ["+user_ref+"] entered Lobby ["+lobby_ref+']');
		};
			...
			io.sockets.on('connection',function(socket){
				...
				socket.on(NEW_USER,function(username,lobby_ref){
					...
					if(lobby != null){
						...
						socket.join(lobby_ref);
						console.log(username+" joined the room "+lobby_ref);
					}
					else{
					...
					socket.emit(LOBBY_INFO, toto.get_lobbies_data());*/
					console.log("++++++++++++ Avant l'émission du message LOBBY_INFO +++++++++++++++");
					//socket.to(lobby_ref).emit(LOBBY_INFO, toto.get_lobbies_data());
					io.sockets.to(lobby_ref).emit(LOBBY_INFO, toto.get_lobbies_data());
					console.log("++++++++++++ Après l'émission du message LOBBY_INFO +++++++++++++++");
				});
			});
